              ...
              Row(
                ...
                children: [
                  TextButton(
                    ...
                    onPressed: () async {
                      // Check if the selected status and reason are valid
                      if (selectedLeadStatus == 'Update Status' ||
                          selectedReason == 'Reason') {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                                'Please select a valid status and reason.'),
                      ...
                      try {
                        // Find the index of the selected status in the leadStatusOptions list
                        int statusIndex =
                            leadStatusOptions.indexOf(selectedLeadStatus!);

                        // Get the corresponding status ID from leadStatusIds list
                        int selectedStatusId = leadStatusIds[statusIndex];

                        // Find the index of the selected reason in the reasonOptions list
                        int reasonIndex =
                            reasonOptions.indexOf(selectedReason!);

                        // Get the corresponding reason ID from reasonIds list
                        int selectedReasonId = reasonIds[reasonIndex - 1];

                        // Update the lead substatus
